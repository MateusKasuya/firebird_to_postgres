...
class ExtractLoadProcess(DbEngine):
    ...
    def change_data_capture(self, df: pd.DataFrame, column: str) -> pd.DataFrame:
        """
        ...
        df_cdc = df.loc[(df[column].dt.date >= ontem) & (df[column].dt.date <= hoje), :]
        print(f'{df_cdc.shape[0]} novas linhas foram detectadas')

        return df_cdc

    def load_to_destination(self, engine: Engine, df: pd.DataFrame, table: str, write_mode: str = "append"):
        """
        ...
        try:
            with engine.connect() as conn:
                df.to_sql(name=table, con=conn, if_exists=write_mode, index=False)
        except SQLAlchemyError as e:
            raise ConnectionError(f'Erro ao gravar dados no banco destino: {e}')
