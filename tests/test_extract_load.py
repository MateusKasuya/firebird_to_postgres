...
from src.utils.extract_load import ExtractLoadProcess  # Ajuste conforme a estrutura do seu projeto
...
def test_change_data_capture(extract_load):
    ...
    df = pd.DataFrame({
        "id": [1, 2, 3, 4],
        "data": [pd.Timestamp(ontem), pd.Timestamp(hoje), pd.Timestamp("2023-01-01"), pd.Timestamp(hoje)]
    })
...
def test_load_to_destination(extract_load):
    ...
    with patch("pandas.DataFrame.to_sql") as mock_to_sql:
        extract_load.load_to_destination(mock_engine, df, "dest_table", "append")
        mock_to_sql.assert_called_once_with(name="dest_table", con=mock_conn, if_exists="append", index=False)
...
def test_load_to_destination_failure(extract_load):
    ...
    with patch("pandas.DataFrame.to_sql", side_effect=SQLAlchemyError("Erro ao inserir dados")):
        with pytest.raises(ConnectionError, match="Erro ao gravar dados no banco destino: Erro ao inserir dados"):
            extract_load.load_to_destination(mock_engine, df, "dest_table", "append")
